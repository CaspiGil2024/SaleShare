      ...
      <div dir="rtl" className="w-full max-w-lg max-h-[80dvh] rounded-2xl bg-white dark:bg-slate-800 shadow-xl flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <Wrench size={18} className="text-orange-500 dark:text-orange-400" />
            תקלות תחזוקה פתוחות
          </h3>
          <button
            type="button"
        ...
        <div className="overflow-y-auto p-3">
          {isLoading ? (
            ...
            <p className="p-10 text-center text-sm text-slate-400 dark:text-slate-500">אין תקלות פתוחות כרגע.</p>
          ) : (
            <ul className="flex flex-col divide-y divide-slate-50 dark:divide-slate-800">
              {rows.map((r) => (
                <li key={r.id} className="py-3 px-2">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{r.summary}</p>
                  <p className="text-sm text-slate-600 dark:text-slate-300 mt-0.5 line-clamp-2">{r.description}</p>
